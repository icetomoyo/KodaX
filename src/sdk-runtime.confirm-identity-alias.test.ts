...
it('rejects an identity confirmation without its persisted delivery receipt', async () => {
...
it.each(['unique', 'conflicting duplicate'] as const)('handles %s delivery identity while preserving caches and replay across restart', async (receiptKind) => {
  ...
  try {
    ...
    // T26 retired the durable journal; the persisted Run status projection
    // is the delivery receipt the Host verifies confirmations against.
    const delivered = (await runtime.runs.get(run.runId)).interruptInputs?.[0];
    ...
    let input = { sessionId: session.id, sourceEntryId: source.id, targetEntryId: LEGACY_IDENTITY_TARGET_ID,
      ...
      delivery: { runId: run.runId, inputId: delivered.inputId, eventId: 'attested-live-event' } };
    if (receiptKind === 'conflicting duplicate') {
      const statusPath = path.join(root, '.kodax', 'runtime', 'runs', encodeURIComponent(run.runId), 'status.json');
      const originalStatus = await readFile(statusPath, 'utf8');
      const parsed = JSON.parse(originalStatus) as { interruptInputs?: unknown[] };
      const conflicting = { ...parsed, interruptInputs: [...(parsed.interruptInputs ?? []),
        { ...delivered, entryId: 'entry_competing_delivery' }] };
      try {
        await writeFile(statusPath, JSON.stringify(conflicting, null, 2));
    ...
    expect(record.data).toMatchObject({ sourceEntryId: source.id, targetEntryId: LEGACY_IDENTITY_TARGET_ID,
      deliveryWitness: { ...input.delivery, receiptDigest: expect.any(String) } });

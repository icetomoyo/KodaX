...
it('keeps real Runtime interrupt delivery resolvable through context saves, compaction, restart and pages', async () => {
  ...
  try {
    ...
    // The durable event journal was retired (T26); the run status projection
    // carries the same delivered-interrupt identity.
    const deliveredEntryId = (await runtime.runs.get(run.runId)).interruptInputs?.[0]?.entryId;
    ...
    const restartedStatus = await runtime.runs.get(run.runId);
    expect(restartedStatus.interruptInputs?.[0]?.entryId).toBe(deliveredEntryId);

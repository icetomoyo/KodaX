...
describe('Runtime daemon capability upgrade', () => {
  ...
  it.skipIf(process.platform !== 'win32').each([1, 2, 3, 4, 5, 6])(
    'keeps a busy sandbox v%i daemon fenced behind the sandbox v7 upgrade requirement',
    async (sandboxVersion) => {
      const calls: string[] = [];
      const oldTransport = createLegacyTransport({

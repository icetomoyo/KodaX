...
import { awaitLatestCodingMemoryReviewDrain, getActiveExtensionRuntime } from '@kodax-ai/coding';
...
class ElicitProvider extends KodaXBaseProvider {
  ...
  async stream(...args: Parameters<KodaXBaseProvider['stream']>): Promise<KodaXStreamResult> {
    ...
    const calledMcp = args[0].some(message => Array.isArray(message.content)
      && message.content.some(block => block.type === 'tool_use' && block.name === 'mcp_call'));
    if (!calledMcp && args[0].some(message => message.role === 'user'
      && typeof message.content === 'string' && message.content.startsWith('/elicit-command'))) {
      return {
...
it.each(['checked-tool', 'sa-continuation'] as const)('retains the shared MCP Session context through a managed command %s', async (delivery) => {
  ...
  try {
    const extensionPath = path.join(homeDir, 'elicit-command.mjs');
    await writeFile(extensionPath, `export default api => api.registerCommand({
      name: 'elicit-command', description: 'Elicit through a managed command',
      handler: async args => args[0] === 'checked-tool'
        ? { message: String(await api.getExecutionScope().invokeTool('mcp_call', { id: '${TOOL_ID}', args: { text: 'please' } })) }
        : { invocation: { prompt: 'Elicit from the shared server.' } }
    });`);
    ...
    try {
      ...
      try {
        const run = await first.commands.execute({
          sessionId: active.id, inputId: 'elicit-command-input', name: 'elicit-command', args: [delivery],
        });
        if (run.kind !== 'started') throw new Error('Expected a managed command Run.');
        ...
        const result = await runtime.runs.await(run.runId);
        expect(result.phase).toBe('completed');
        expect(delivery === 'checked-tool' ? result.result?.lastText : results.join('\n')).toContain('elicit:accept:kodax-shared-answer');

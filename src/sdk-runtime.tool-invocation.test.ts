...
it('executes explicit tools from daemon negotiation with permissions, events and durable history without a model', async () => {
  ...
  const initialized = await transport.request('initialize', { profile: 'default', token: readRuntimeDaemonToken(paths),
    ...
    identity: typeof runtime.identity; capabilities: Readonly<Record<string, unknown>>; journalEpoch: string;
  };
  ...
  try {
    ...
    const seen: string[] = [];
    const subscription = client.events.subscribe({ sessionId: session.id }, event => { seen.push(event.type); });
    ...
    try {
      ...
      const events = await client.events.replay({ sessionId: session.id });
      expect(events).toEqual(expect.arrayContaining([
        expect.objectContaining({ runId: read.runId, type: 'tool.finished' }),
        expect.objectContaining({ runId: denied.runId, type: 'tool.finished' }),
      ]));
      ...
      try {
        ...
        expect(await restarted.events.replay({ sessionId: session.id })).toEqual(events);

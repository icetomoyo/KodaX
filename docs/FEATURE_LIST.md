...
| Item | Value |
|---|---|
| Current released version | `v0.7.96-rc.2` (Git tag / GitHub pre-release) |
| Current package version | `@kodax-ai/kodax@0.7.96-rc.2` (npm publication remains manual) |
| Workspace baseline | `llm / agent / coding / repl` 4 packages |
| Current implementation | [FEATURE_299 v0.7.96](features/v0.7.96.md#feature_299-unified-stop-full-access-and-extension-execution-contracts) — released in `v0.7.96-beta.9`, extended in `v0.7.96-rc.1` and `v0.7.96-rc.2` |
...
`v0.7.96-rc.2` is the second release candidate of the v0.7.96 line. A first
...
adapters, with MCP images persisted as attachments and image-capable OpenAI
providers receiving tool images after valid paired responses. Local
execution failures and Worker error codes carry typed classification through
child results, Actor metadata, and Runtime/daemon status. Delivered
interrupt entry references stay valid across context rewrites, storage
reads/saves, compaction, restart, and conversation pagination. npm
publication remains a manual maintainer step.

## v0.7.96-beta.6 Release Record

`v0.7.96-beta.6` was the sixth beta pre-release of the v0.7.96 line: a
...
`deepseek-v4-anthropic` reasoning preset; legacy `deepseek-v4-pro` remains
selectable. Replayed restored-session histories answer orphaned `tool_use`
calls with an explicit interrupted-tool marker, Anthropic serialization keeps
assistant block order, and typed `AbortError` cancellation takes precedence
over provider text matching. Windows sandbox setup advances to generation 11
(Issue 333): Codex-compatible profile/SSH ACL exclusions with setup-only,
ownership-exact generation-10 cleanup that retains old SID/nonce/roots across
retries. npm publication remains a manual maintainer step.

## v0.7.96-beta.4 Release Record

`v0.7.96-beta.4` was the fourth beta pre-release of the v0.7.96 line.
...
`v0.7.96-beta.2` is the second beta pre-release of the v0.7.96 line. It
retains the full beta.1 contract and adds long-session context-boundary
recovery plus Provider correctness fixes: a confirmed upstream context
overflow permits one generation retry after persisted history reduction
without replaying tools, protected tool-output previews persist as durable
artifacts, bundled Provider credential scopes stay shared across SDK, Agent
summaries, CLI, and Runtime Worker requests (Issue 332), structured Child
Agent Provider failures survive Actor settlement (Issue 330), custom Provider
credential verification honors exact run-scoped authority (Issue 331), and
tool-free side queries omit empty `tools` arrays for strict vLLM gateways
(Issue 329). npm publication remains a manual maintainer step.

## v0.7.96-beta.1 Release Record

`v0.7.96-beta.1` was the first beta pre-release of the v0.7.96 line. It
retains
the full alpha.7 contract — FEATURE_297 permissions, the ADR-070/Issue 326
Windows concurrency stabilization (`sandboxRuntime:11`, native protocol/setup
generation 10), interactive stale-setup self-healing, responsive recovery
status, and host-control-plane doctor guidance — and adds the Issue 328 Linux
glibc 2.28 fix: x64/arm64 FEATURE_295 native text authorities compile with
Rust 1.98.0 in digest-pinned `manylinux_2_28` builders, and CI/release lanes
reject any staged addon requiring a `GLIBC_*` symbol newer than 2.28. npm
publication remained a manual maintainer step.

## v0.7.96-alpha.7 Release Record

`v0.7.96-alpha.7` completes the post-alpha.5
...
`sharedSessionSettings:2` remain the released contract. Interactive Windows
...
host terminal or `/sandbox`, not through Bash. npm publication remained a
manual maintainer step. Embedded native manifests retain their exact
...
`v0.7.96-alpha.6` is the GitHub pre-release at `a71d2b98`. It contains the
protocol-10/setup-generation-10 admission and concurrency correction, exact
native generation pinning, explicit execution-proving doctor, host-boundary
authorization forwarding, and the bare `kodax -r` input handoff fix. It
predates alpha.7's interactive stale-setup self-healing, responsive recovery
status, and host-control-plane doctor guidance. npm publication remained manual.

## v0.7.96-alpha.5 Release Record

`v0.7.96-alpha.5` is a GitHub pre-release. It ships `FEATURE_297` and
stabilizes Issue 326: four canonical permission profiles (Plan, Edits,
Auto[LLM], Full Access) replace the legacy Plan/Edits/Auto[RULES]/Auto[LLM]
...
| ID | Title | Category | Priority | Planned | Design |
|---|---|---|---|---|---|
| `298` | Product Host and Client Contract Simplification | Core / Runtime + UI SDK + Execution | Critical | `v0.7.97` | [v0.7.97](features/v0.7.97.md#feature-298) — specification / ticket review |

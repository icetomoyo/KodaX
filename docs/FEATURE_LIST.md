...
- `FEATURE_104` (v0.7.29) 是 prompt-eval 测试基础设施模块化：把 v0.7.29 之前 7 个 `tests/*.eval.ts` 各自 inline 的 `(provider, model, apiKeyEnv)` 三元组收敛到 `tests/prompt-eval/aliases.ts` 的 8 个用户指定 coding-plan 短名（`zhipu/glm51` / `kimi` / `mimo/v25{,pro}` / `mmx/m27` / `ark/glm51` / `ds/v4{pro,flash}`）；提供共享 judges (`mustContainAll/Any` / `mustNotContain` / `mustMatch/NotMatch` / `lengthWithin` / `parseAndAssert` / `runJudges`) 和 harness (`runOneShot` / `runABComparison` / `formatComparisonTable`)；把 "什么 PR 必须配 prompt eval" 的判断口径写进 `tests/prompt-eval/README.md`（凡涉及 system-prompt / role-prompt / tool-description / coding-preset DEFAULT_INSTRUCTIONS / protocol-emitter prompts 的字符串字面量都要 eval；只调思考深度参数的不要）。27 个 self-test 锁住 alias 表 verbatim shape + judge 行为 + comparison aggregation，跑在默认 `npm test` 里零 LLM 成本；真实 `*.eval.ts` 跑在独立的 `npm run test:eval`（缺 API key 自动 skip）。详见 [v0.7.29 FEATURE_104 设计](features/v0.7.29.md#feature_104-prompt-eval-harness-module)。

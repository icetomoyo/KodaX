...
See the
[main-branch regression evidence](test-guides/ISSUE_337_v0.7.96_REGRESSION_GUIDE.md#main-branch-backport-and-integration-evidence).
...
## Resolved in working tree 2026-09-12 — 多客户端设置显示同步
...
Current markers use a marker-only startup fast path; when a real migration runs
in a child process, `kodax -r` and new-session startup keep a live elapsed-time
indicator instead of freezing the picker. Sandbox doctor is a host control-plane
command: use `/sandbox` inside the REPL or run `kodax sandbox doctor` directly
in a user terminal. Running it through Bash diagnoses from inside the restricted
account and can report a context-local `state.db` access denial even while
...
artifact for the current content hash is published atomically on first use;
publication constructs and verifies owner/DACL state, while every ordinary
command verifies stable identity, physical path, and hash locally without a
PowerShell subprocess. Invalid existing content remains fail-closed. The host

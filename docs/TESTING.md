...
KodaX 包含多个自动化测试，分布在各个 package 中：
...
│           └── __tests__/
│               ├── fuzzy.test.ts
│               ├── skill-completer.test.ts
│               ├── argument-completer.test.ts
│               └── autocomplete-provider.test.ts
├── ai/
├── agent/
├── skills/
└── coding/
...
# 只运行 REPL 包中的测试
npm test -- --project repl

# 运行特定测试用例
npx vitest run -t "fuzzy"
```

> 注意：测试文件位于各个 package 内部，不再使用统一的 `tests/` 目录。
...
kodax --provider zhipu-coding "
在 packages/coding/src 中搜索 'async' 出现的次数
"
...
```bash
# 测试上下文压缩（需要较多消息触发）
kodax --provider zhipu-coding --session compress_test "
请执行以下步骤：
...
```bash
# 1. 首先初始化长运行项目
kodax --provider zhipu-coding --init "构建简单的 TODO 应用"
...
KodaX 是 TypeScript 实现的，以下是一些特有功能：

### 1. read 工具 offset/limit 参数

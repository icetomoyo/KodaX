...
> 2026-09-08 更新：用户已明确授权继续按设计修复，并在修复后合并主仓库最新进展。下文 2026-09-05 的停止指令及后续 GLM 门禁仅是历史记录，不再代表当前状态。本轮复查整改和实际验证以 [REVIEW_v0.7.97_FINAL.md](REVIEW_v0.7.97_FINAL.md) 的 2026-09-08 节及 [正式票据](features/v0.7.97.md) 为准。修复 `530e0785` 后已合入主线 `17474b64`，自动门禁已通过，人工发布验收边界见评审收尾记录。尚未推送或发布。
...
**T21 补充侦察（同日追加）**：(1) A2AServerConfig 无 listen 字段（仅 publicBaseUrl；a2a/config.ts:152-160）——托管需新增 bootstrap listen 配置（{hostname,port}）或由 daemon serve owner 参数携带；(2) createA2AServerOptionsFromConfig（a2a/product.ts:79）以外部 listenBaseUrl 构造 options，托管侧可直接复用；(3) 托管挂点=kodax_cli.ts:1080-1235 daemon serve owner：createRuntime 回调内已有 a2aIntegration.start(runtime)（outbound），serving 在此后加——if (serverBlock.listen) prepareKodaXA2AServer(createA2AServerOptionsFromConfig({runtime: ownedRuntime,...})) + listen + finally close；binding 变更 restart-required 由 classifyA2AServerChange 已有分类保证（活 Host 只热更 published/limits，execution/listen 改动=重启）；(4) `kodax a2a serve`（runA2AServeCommand）退役路径：写 listen 进 config → 提示经 daemon 托管（或保留为薄壳）——注意 integration-cli.a2a-serve.test.ts 与 extensionRuntime/outbound integration 的去留评估。

## 2026-09-07 T21 完成（31/35）：A2A prepared serving 由 daemon Host 持有

**切片与提交**：S1 `4e2e6e74`（A2AServerConfig 增 bootstrap-only `listen:{hostname,port}`（loopback-only、port 1-65535、listen 入 restartFields）；daemon serve owner（serveDaemonCommand）createRuntime 内 fail-tolerant 读 server 块、有 listen 则 prepareKodaXA2AServer+listen；kodax_cli.daemon-smoke 真实 spawn Host S1）；S2 `b387f4f1`（`kodax a2a serve` 改薄壳：assertLoopbackHostname→require delegate→assertA2AConfigOwnerCompatible→setA2AServerConfig 持久化 listen→委托 serveDaemonHost（依赖注入回调避免 kodax_cli↔integration-cli 循环）；Host 内热重载：IntegrationConfigController 订阅→classifyA2AServerChange→hot 字段 updateHot、restart-required 打 pending 警告；删 createA2AServerExtensionRuntime 与 serveA2A 旧 embedded 体系全部孤儿 import；a2a-serve.test 重写为委托契约 5 测）；S3 `b34d85fb`（src/a2a/hosted-serving.test.ts S1）+ `82d039e0`（双轴评审修复）。docs/features submodule 两笔（翻牌+指针）。

**S1 证据（四票面子句全覆盖）**：(1) 允许范围成功+越权拒绝+身份归属+workspace 隔离——hosted-serving.test.ts：真实 createKodaXRuntime + 真实 runtime.agents.execution binding service，Object.create(realExecution) 观察代理（闭包对象，prototype 委托安全）仅记录 bindDefault 输入（policy 原样到达）与 startDefault 的 agentContext（全部 = `a2a:${realmA2APrincipalKey(realm,subject).slice(0,16)}`），执行本体不变；scripted SSE provider：允许文本任务 COMPLETED；'Outside-range work' 触发 bash tool_call → guardrail block（'[Tool Error] Blocked by guardrail … Tool is outside the bound surface' 回传 provider 二次请求）、marker 文件从未落盘；ctx-a/ctx-b 独立 workspaceRoot（同 context 复用同 root）、图片 raw part 物化进 ctx-b inbox 且 base64 到达 provider wire。file 类 artifact 被 SDK 运行时按设计拒绝（MODEL_INPUT_UNSUPPORTED，validation.ts）——附件走 image 路径。(2) 热重载/重启语义 e2e——daemon-smoke hosted 测试扩展：setA2AServerConfig 改 published.name → live card 改名；listen 改写其它端口 → 原 card 仍在、新端口不 bind。(3) mutation 验证：startDefault 的 guardrails 置空 → 测试红（注意 1350 行是 startLocal，A2A runtime-default 走 1229 startDefault）。

**双轴评审**：Spec PASS（无新 IPC、production 只剩 kodax_cli 一个 prepareKodaXA2AServer 调用点、restartFields 保证 listen/execution/dataDir 不可热更、bootstrap 显式启用）。Standards 3M/6L 全修：serveA2A delegate 前置校验先于配置写入；三处 teardown 统一 closeHostedA2AServing（controller close 受保护）；smoke 子进程 stderr 排水；server 块删除打 pending-restart 告警；busy port fail-fast 注释化；IPv6 方括号渲染；测试 socket error 注释与逐步受保护清理。未采纳：permissive 对照例（bash 无 remoteContract，绑定面根本不收 bash，无法构造）；already-owned 提示增强（Low，Host 自有输出）。

**回归**：build 绿；a2a 全套 217、daemon-smoke 25/25（~260s）、integration-cli 28、hosted-serving 1、runtime-agent-binding 15；parity 415 无新漂移（噪声类别同前）。已知残留：daemon-smoke 首跑偶发 1 败（负载 flake，重跑稳定绿）。

**下一步（31/35）**：T25（control journal 全退役——agent 族已豁免 envelope；按票删除 auto-generated operationId）；然后 T26（前置 T15/T21 均只差 T15）；T27；T15 验收（需 T25）。DAG：T25→T26→T27→T15。

## 2026-09-07 T25 调研地图（未实施，接续点）

**票面**：所有 mutation 退出通用 control journal；删除 generic operation envelope/receipt/operations.get、自动生成 operationId、control journal 存储/恢复/全局隔离及相关协议、schema、capability；不能移到私有目录。以 RPC 矩阵和非 RPC 产品入口表核对 Memory、命令/Skill、recover、临时会话、Agent、compact、goal。T15 附带条款：exit-settlement/stopForInline/rollbackToInline/prepared-exit 票据消费的删除挂在 T25。

**命名警示**：control journal epoch（je_<uuid>，control-journal.ts）是 T25 目标；session event journal epoch（RuntimeSessionCursor.journalEpoch，sdk-runtime.ts:2731）是 T26 地盘，不可动。

**核心**（src/runtime-daemon/control-journal.ts，418 行，整文件删）：journal-meta.json + control.jsonl 存储；recoverUnfinished 把 accepted→interrupted/dispatched→unknown；execute 按 operationId dedup+replay；两处隔离置 control_history_untrusted 后本进程全部 execute 拒绝（含 daemon.stop/shutdown/rollbackToInline 被栅栏——即票面"全局隔离"）。exit-settlement.ts:433-443 每次结算铸新 stopOperationId 传给 rollbackToInline（envelope-required）。

**消费面**：host.ts:84-86/113/117 唯一构造（rootDir/control）+ requireOperationEnvelope:true；server.ts dispatchWithOperation:851-908 豁免链（非 mutation/input.submit/withdraw/session.settings.update/agent 族）→ capability 检查 operationDeduplication → operation_required → controlJournal.execute；operation.get 处理 :1119-1137；initialize 带 journalEpoch :1045；capability 广告 operationDeduplication/journalEpoch/controlHealth :2602-2611 + durableRecoveryQueries.operationResult :2674-2686；publicOperationReceipt :2779。protocol.ts envelope 类型 :308-314/校验 :660-675/:756/:833-840、operation.get :18/:354、operation 错误码 :277-282/:640-645。schema.ts:125-131 operation.get。client.ts 自动 envelope :273-282 + createOperationEnvelope :1557-1567（自动 operationId=op_<uuid>，票面点名删除）+ 各方法显式转发（sessions.create:512、updateSettingsVersioned:650、compact:688、runs.start:716、submitInput:730、rollbackToInline:1370）+ operations face :993。transport.ts fallback 自动 envelope :391-400 + epoch 捕获 :453 + withDurableOperationCapability 注入 :504-525（sdk-runtime :6206 同）。sdk-runtime.ts RuntimeOperationOptions/Receipt/Service :996-1026、operations face :1079、embedded stub :5203-5209、requirement 断言 :5471/:5522。index.ts:126-127 公共导出。run-origin operationId 镜像 server.ts:1862/1928/1942 + sdk-runtime.ts:18178/18394（compact credential target 'operation' Host-minted compact_<uuid> 是 T31 的，与 envelope 无关，保留）。

**关键事实**：(1) operations.get 无任何生产消费者（仅测试 stub）；(2) goal 族（session.goal.create/pause/resume/clear）在 mutation TYPE union 里但不在运行时数组——实际已 envelope-free，T25 顺手清类型；(3) Memory 无 daemon mutation RPC（票面该行指非 RPC 入口）；(4) reverse-bridge（credential.register 等）本就不在 journal；(5) 剩余 envelope-required 清单=RUNTIME_DAEMON_MUTATION_METHODS 数组减豁免（daemon 生命周期/session 生命周期/runs/permission/interaction/workflow/learning/config/provider/mcp/extension/artifact 各域）。
...
## 2026-09-07 T26 调研地图（Explore 侦察，未实施）

**票面**：删除 Runtime durable event seq/epoch、replay、journal 索引及 terminal/input 反向修复；保留 Actor/A2A 领域事件与单次 JSON 非持久诊断适配（不得反向裁定业务状态、不得参与运行 admission）；不删 canonical Session/Run 数据；不得把持久 UI journal 改名 trace。Appendix B（v0.7.97.md:981-983）：event.subscribe/unsubscribe/replay → T26，"当前视图替代"。

**A 持久化面（全删）**：sdk-runtime.ts createRuntimePersistence :15011-15032 — runs/<runId>/events.jsonl（账本）、events.watermark（v2 保留水位，trimEventFile :15439）、event-journals.json（journal 索引，{sessionId,journalEpoch} 列表）、session-events/<sid>/sequence（seq 文件，writeRuntimeSequenceCursor :16732）、session-events/<sid>/journal.json（epoch 元数据，readSessionJournalEpochLocked :15156 mint randomUUID）。写链：createRuntimeEventBus :13915（50ms coalesce :3947、16MiB/8MiB trim、10k 内存）→ flushSession :14168 → commitEvents :14039 → commitEventBatch :15611（锁内 reserveEventSeqsLocked :15191，floor 扫描 findMaxPersistedEventSeq :15080 全 run 账本尾扫）→ appendEventBatch :15527（尾部修复 :15471、索引重建、truncate 回滚、RuntimeEventCommitIndeterminateError 终锁 :14057、背压锁 :14185）。emitDurable :14614 同步持久——两个业务定序用户：run.input.delivered :10681（afterPersist 才置 queue-record delivered）与 publishUnconfirmedRunUpdate :9181。终态路径 settleTerminalRun :22996：status.json 先写、事件后发（权威已对齐 T19）。epoch 镜像进 status.json._runtime :16450、仅 aggregate replay 根过滤消费 :16346。

**B replay 面（全删）**：persistence.replay :16322（全 run 目录扫描、child-journal epoch 过滤、cursor 校验 resync_required、watermark 保留检查 :15711）；bus service.replay :14509 + RuntimeEventService.replay :14553（接口 :2712）；sessions.observe :8213-8352 = live 订阅+缓冲 256+快照（captureObservationSnapshot :7359，sameRuntimeSessionCursor :14961 一致性环）——重连无 replay（invalidation 全量快照、epoch 不符 runtime_changed）。conversation/transcript 读不经 journal（canonical storage，:7903/:8165/:8195、session-view.ts:339）。epoch 生命周期：retire/restore/prepareSessionJournal :7597/:7602/:7690（重建→新 epoch+seq 归零，驱动 observe runtime_changed）。

**C 反向修复残留（T33 后）**：内存 live 投影 applyRuntimeSessionEvent :14751（pendingUserInputs 由 user_input.requested/resolved/终态维护——仅显示，不落盘，保留）；eventRunSessionId :16291（status 缺失时从账本推 run 归属——删）；**admission 耦合：assertAdmittedEventScope server.ts:2667-2688 用 events.replay({runId,limit:1}) 作 Run status 缺失时的事件域 admission 探针——票面点名必须解耦（改 status-only，缺失即拒）**。

**D wire/capability 面**：protocol.ts event.subscribe/unsubscribe/replay :72-74/:389-391；schema.ts :341-346、runtimeEventSchema :1725（cursor 必填）、runtimeSessionCursorSchema :1739（journalEpoch 必填）、诊断 record observation.cursor :1481；server.ts dispatch :1878-1912、filterReplayForClientCapabilities :2652；capability 广告 sessionEventJournal :2406（cursor session_epoch_sequence）、liveOutputSegments :2412（rawJournal:"complete"）、runtimeEventCoalescing :2418、sessionObservation :2471、typedRuntimeEvents :2485；client.ts events face :744-757、subscribeToDaemonEvents :1528、subscribeToDaemonNotification :2100、observeDaemonSession :1868（客户端 epoch/序检查 :1725-1748）；sdk-runtime 硬门 4162-4163（daemon connect 必需 sessionEventJournal:1+liveOutputSegments:1）、required map :900-913、断言矩阵 :5401-5460、embedded 广告 :4268-4281；runtime-event.ts parse :37-97（seq+epoch 校验）；index.ts 导出 :39/:62-72/:148。

**E 消费者分类**：需历史(replay)仅 3 处——event.replay RPC 本身；诊断服务 createRuntimeDiagnosticsService :13603-13627 + latestRuntimeDiagnosticPayload :13629-13686 → context.budget.get/tool.exposure.preview/provider.cache.diagnostics.get（票面"真实独立诊断消费者"，需明确边界或转 live-cache）；assertAdmittedEventScope 探针。live-only（保留）：kodax_cli:638、acp_server:373/:1223、run-progress-events:408（T35 单次非持久）、a2a/server:1199 attachRuntimeEvents（T33 live+快照）、内部 sessionViews.changed :4466、settings 发布 :6212。视图面（无事件）：plane.observe/classic-display/observeView；session-view uiHistory=canonical（禁改名 trace）。保留清单：Actor 领域事件（agents.events :13487 root.eventSnapshot 纯整数序列）、agent-store events.jsonl（域自有）、workflows run-graph、status.json/run-status-index、REPL storage epoch（拓扑 epoch 无关）。

**G 钉住测试**：sdk-runtime.session-events.test.ts 整套 25 例（序列/cursor/epoch/水位/截断恢复——大删）；sdk-runtime.test.ts ~174 处（:643 daemon 门、:925 coalesce 门、:2521 账本不可读恢复、:15976-16228 水位 v2 族、:9193/:9300/:9357 终态权威已 T26 对齐）；server.test.ts :207/:2356/:2386/:2604；client.test.ts :569-651/:717-832/:1235-1299；schema.test、runtime-event.test、shared-daemon:278、daemon-smoke:1014（events.replay 作观察探针）；self-knowledge registry :1322/:1533-1543。

**H 切片草案**：S1 durable 机制关闭+live 流保留（envelope 去 seq/epoch 或仅留内存 seq；emitDurable→emit，afterPersist 定序语义保持——持久化消失后同步 emit 后即 mutate；observe 一致性环换 change-token（transcriptRevision 或单调计数）；coalesce 随磁盘写一起删）。S2 wire+capability（event.* 3 RPC 删——live 消费者钉住改走 sessions.observe 事件通道或 SDK face 内部订阅；assertAdmittedEventScope 改 status-only；sessionEventJournal capability+硬门删；liveOutputSegments/rawJournal、runtimeEventCoalescing 处置；observe 快照 cursor 字段处置；诊断转 live-cache 明确非 admission 边界）。S3 测试/docs/残留（session-events 套件大删改写、水位族、daemon-smoke 探针、self-knowledge 退役注记；磁盘遗留文件不迁移不重写（T33 先例））。

## 2026-09-07 T26 实施设计（侦察后定稿）

**架构决策**：(1) envelope 保留进程内 per-session 单调 seq（emit 顺序赋号，retire/prepare 重置）作 live 排序提示——票面删除的是 *durable* seq/epoch；删 cursor 对象/journalEpoch/RuntimeSessionCursor 类型；snapshot.cursor→snapshot.seq（捕获时高水位，"seq<=此值已反映进快照"）。(2) merge/50ms 窗口保留（流式背压工具非持久化工具；flush 腿改为内存 createEvents+projection+notify）；runtimeEventCoalescing capability 保留；sessionEventJournal capability 删除。(3) RuntimeEventFilter 收窄：sessionId 必填（runId/type 可选本地收窄）——resolveRuntimeEventScope 删除；唯一 runId-only 消费者 acp_server watchRuntimePermissionSettlement 改 {sessionId: request.sessionId, runId}（RuntimePermissionRequest 自带 sessionId）。(4) wire：event.subscribe/event.replay 删除；event.unsubscribe 改名 subscription.close（observe/view 两处客户端 :1684/:1847 依赖它作通用关闭通道，server case 是共享 closeSubscription）；客户端 events face 仅存 subscribe，重锚 session.observe（忽略快照、转发 event 通知、observation.invalidated→关闭、本地 runId/type 过滤）；pending 通知缓冲语义复用 subscribeToDaemonNotification 模式。(5) 诊断面整删（零生产消费者）：RuntimeDiagnosticsService/createRuntimeDiagnosticsService/latestRuntimeDiagnosticPayload（sdk+server 两份）/context.budget.get+tool.exposure.preview+provider.cache.diagnostics.get RPC/contextDiagnostics capability/filterReplayForClientCapabilities；诊断事件类型保留发射。(6) emitDurable→emit（afterPersist 同步后跑）；assertAdmittedEventScope 随 event.* RPC 消失（session.observe 走 assertAdmittedSessionId + 客户端 runs.get 解析 run）。(7) 磁盘：停写 events.jsonl/events.watermark/event-journals.json/session-events/*；存量文件不迁移（T33 先例）；status.json sessionJournalEpoch 镜像删。(8) 观察失效：retire/prepare 显式 invalidateSessionObservations("runtime_changed")，旧订阅者不再依赖 epoch 轮换检测。
...
## 2026-09-07 T27 调研地图 + 实施设计（Explore 侦察后定稿）

**票面**：删除产品 embedded/worker facade、旧 mode/isolation options、专属 sidecar/exports 与无消费者 callback；保留 constructed-handler 隔离 Worker 与 Host A2A outbound/bootstrap/watchers；embedder-guide v0.7.96 快照 operation/settlement 章节退役（T25 顺延）；request.cancel/ack 仅留实际传输控制（Appendix B :979-980）；迁移说明 + 发布验收（低层包独立导入运行）。粒度：不允许 legacy fallback 让测试变绿。

**关键裁决（防过度删除）**：(1) **所有 SDK 子路径出口保留**——guide 显示每个 subpath（agent/llm/coding/repl/a2a/media/session/skills/mcp/sandbox/client/runtime/experimental-memory）都有文档化的外部 embedder 消费（KodaX Space）；"专属 sidecar/exports"= 专属于被删 worker facade 的 sidecar 与出口，不是全面出口清洗；"低层包独立导入"用独立导入测试验收而非删 re-export。(2) **mode 选项保留**：embedded|daemon 是现存产品模式对（CLI --runtime-mode + T02 启动器 + autoStartDaemon 依赖 daemon attach，删除即退步）；删除的是 isolation 维度（'worker' 唯一公共用途）与 worker 选项。(3) **owner-mode 家族导出保留**（acquireKodaXInlineOwner/setKodaXRuntimeOwnerMode/getKodaXRuntimeOwnerState/enableKodaXDaemonOwner/handleRuntimePermissionRequest）——T02/T25 契约 + guide 文档化；仅删 getKodaXRuntimeOwnerPolicy（全仓零消费者）。(4) **runtime-worker/ 整目录删除**（transport/entry/protocol + dist/runtime-worker.js sidecar + build-bundle 条目）。

**S1 worker facade 删除**：src/runtime-worker/ 三文件；sdk-runtime.ts createWorkerHostedKodaXRuntime（:5179-5330）+ CreateKodaXRuntimeOptions.isolation/worker（:795-798）+ 校验分支（:4016-4031 worker 部分、:4096-4105）+ RuntimeWorkerOptions 再导出（:462-465）+ close() 注释（:1025）；KodaXRuntimeIsolation 收窄 'inline'|'process'（identity 事实保留，'worker' 消灭）；kodax_cli.ts workerHostedEmbedded 分支（:4916-4918/:4952-4957/:4981-4983——a2aIntegration 条件去 !workerHostedEmbedded）+ toPreparedRunStartOptions worker 分支（:750-752 transportIsolated=mode==='daemon'）；utils.ts worker.configuredA2A 配置键（:1086/:1365）+ generated-config-templates 模板重生成（scripts/sync-config-templates.mjs）；package.json files 去 dist/runtime-worker.js + build-bundle.mjs 入口。runtimeMode 配置键/KODAX_RUNTIME_MODE/--runtime-mode 保留。

**S2 request.cancel/ack 收窄**：删除 late-result 保留+补投（通用回执/恢复）：transport.ts lateResults Map/removeLateResult/clearLateResults/RUNTIME_DAEMON_LATE_RESULT_RETENTION_MS/abort 时 30s 保留注册（:418-427）/未知 req_ 成功响应的保留投递分支（:292-307 仅留 request.cancel 卫生）+ RuntimeDaemonRequestControl.onLateResult 字段；client.ts readRequest 第 4 参 onLateResult+deliverLateResult+operation.then 补投（:265-322）+ observeDaemonSession 签名收窄。保留：request.ack 每成功响应、abort→request.cancel、未知 req_→request.cancel、server 两 handler、draining 成员资格。验证：全仓无生产 onLateResult 传递者（仅 transport/worker 内部+测试）。

**S3 无消费者面清理 + 发布验收**：CommandCallbacks.setThinking 删（types.ts:192 + repl.ts:1306-1311 + InkREPL.tsx:9980，零测试钉）；src/index.ts getKodaXRuntimeOwnerPolicy 导出删；新 src/sdk-independence.test.ts——@kodax-ai/llm、@kodax-ai/agent、@kodax-ai/coding（repl 为 Ink UI，导入+纯函数级）各自独立导入并运行基本操作；tests/release-workflow.test.ts sidecar 清单更新（去 runtime-worker）；package.json exports/files 终态断言。

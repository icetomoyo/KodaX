...
> **状态**: ✅ Phase 6.3 已完成 | **更新日期**: 2026-02-20
>
> **技术栈**: React + Ink (React for CLI) - 与 Claude Code / Gemini CLI 一致
>
> **参考实现**: [Gemini CLI](https://github.com/google-gemini/gemini-cli)
>
> **最新分析**: 详见 Section 10-13 (KodaX vs Gemini CLI 差距分析)
>
> **最新功能**: Thinking 字符计数、[Thinking]/[Tool] 标记、状态栏模式指示
...
## 10. KodaX vs Gemini CLI 差距分析 (2026-02-20)

> **背景**: 深入研究 Gemini CLI 源码后发现 KodaX Ink UI 存在显著功能差距。

### 10.1 架构对比

| 模块 | Gemini CLI | KodaX | 差距 |
|------|------------|-------|------|
| **主容器** | `AppContainer.tsx` (~2000行) | `InkREPL.tsx` (~570行) | KodaX 过于简单 |
| **输入组件** | `InputPrompt.tsx` (~1500行) | `InputPrompt.tsx` (~250行) | 缺少许多功能 |
| **文本缓冲** | `text-buffer.js` (~130KB) | `text-buffer.ts` (~450行) | 功能差距大 |
| **键盘系统** | `KeypressContext.tsx` (~22KB) | `useKeypress.ts` (~70行) | 缺少优先级系统 |
| **流式处理** | `useGeminiStream.ts` (~57KB) | 无 | 完全缺失 |
| **主题系统** | 多个主题文件 + Manager | `dark.ts` (~50行) | 非常基础 |

### 10.2 严重问题 (P0)

#### 10.2.1 🔴 消息列表不渲染

**现状**: `InkREPL` 只渲染 `InputPrompt`，所有输出通过 `console.log` 打印。
```typescript
// InkREPL.tsx - 只有输入框
return (
  <Box flexDirection="column">
...
**Gemini CLI 做法**:
- `AppContainer` 管理完整 UI 状态
- `HistoryItems` 组件渲染消息列表
- 工具调用有专门的 `ToolGroup` 组件
...
```
- 实时更新 UI 显示部分响应
- 加载指示器 + 短语循环 (witty/informative/minimal)

#### 10.2.3 🔴 无工具执行可视化

**现状**: 工具调用（文件读写、命令执行）无视觉反馈。
...
// 状态图标
const TOOL_STATUS = { scheduled: '○', executing: '○', success: '✓', error: '✗' };
```

#### 10.2.4 🔴 键盘系统过于简单

**现状**: 无优先级系统，无法让不同组件处理相同按键。
...
// 使用 MultiMap 管理订阅者，同优先级内后注册的先处理
```
...
| 功能 | Gemini CLI | KodaX |
|------|------------|-------|
| **Unicode 处理** | LRU 缓存 + code point | 基础 code point |
| **视觉/逻辑光标** | ✅ 分离 | ❌ |
| **粘贴占位符** | `[Pasted Text: X lines]` | ❌ |
| **外部编辑器** | Ctrl+O 打开 $EDITOR | ❌ |
| **文本选择** | ✅ | ❌ |
| **滚动视口** | ✅ | ❌ |
| **大粘贴保护** | >50 行或 >5000 字符折叠 | ❌ |
| **持久化历史** | RecordingService | 仅内存 |
| **主题数量** | 10+ | 2 |
| **自动主题检测** | ✅ | ❌ |

### 10.4 Gemini CLI 关键实现参考

```
packages/cli/src/ui/
├── AppContainer.tsx          # 主容器 (~2000行)
├── components/
│   ├── InputPrompt.tsx       # 输入组件 (~1500行)
│   ├── SuggestionsDisplay.tsx # 补全 UI
│   └── shared/text-buffer.js # 文本缓冲 (~130KB)
├── contexts/
│   ├── KeypressContext.tsx   # 键盘系统 (~22KB)
│   ├── UIStateContext.tsx    # UI 状态
│   ├── UIActionsContext.tsx  # UI 动作
│   └── StreamingContext.tsx  # 流式状态
├── hooks/
│   ├── useGeminiStream.ts    # 流式处理 (~1600行)
│   └── useLoadingIndicator.ts # 加载指示
├── themes/
│   ├── theme.ts              # 主题类
│   └── theme-manager.ts      # 主题管理
└── utils/textUtils.ts        # 文本工具 (LRU 缓存)
```
...
| 问题 | 位置 | 优先级 |
|------|------|--------|
| 键盘逻辑重复 | `useTextBuffer.ts` + `InputPrompt.tsx` | P1 |
| 组件未使用 | `App.tsx`, `MessageList.tsx`, `StatusBar.tsx` | P0 |
| 无持久化存储 | `MemorySessionStorage` | P1 |
| 消息类型简单 | `types.ts` | P1 |
...
| 任务 | 文件 | 描述 |
|------|------|------|
| 6.1 | `contexts/UIStateContext.tsx` | 创建全局状态 Context |
| 6.2 | `contexts/KeypressContext.tsx` | 创建优先级键盘系统 |
| 6.3 | `contexts/StreamingContext.tsx` | 创建流式状态管理 |
| 6.4 | `components/MessageList.tsx` | 重写消息列表渲染 |
| 6.5 | `components/ToolGroup.tsx` | 工具执行可视化 |
| 6.6 | `components/LoadingIndicator.tsx` | 加载状态指示器 |
| 6.7 | `InkREPL.tsx` | 重构集成所有新组件 |

### Phase 7: 文本处理增强 (P1)

| 任务 | 文件 | 描述 |
|------|------|------|
| 7.1 | `utils/textUtils.ts` | LRU 缓存 + code point 工具 |
| 7.2 | `text-buffer.ts` | 视觉/逻辑光标分离 |
| 7.3 | `hooks/usePasteHandler.ts` | 粘贴检测 + 保护机制 |
| 7.4 | `utils/externalEditor.ts` | 外部编辑器支持 |

### Phase 8: 体验优化 (P2)

| 任务 | 文件 | 描述 | 状态 |
|------|------|------|------|
| 8.1 | `utils/terminalCapabilities.ts` | 终端能力检测 | ✅ 已完成 |
| 8.2 | `components/SuggestionsDisplay.tsx` | 自动补全 UI | ✅ 已完成 |

> **注意**: 主题管理和多主题支持暂缓，保持简单的 dark/minimal 两主题方案。
...
#### 测试 1: 启动 Banner 和 StatusBar 显示

```bash
kodax
```
**预期**:
1. 显示 ASCII 艺术 Logo (6 行 KodaX)
2. 版本信息行: `v0.3.2 | <provider>/<model> | code +think +auto` (根据配置)
3. Session ID 和工作目录
4. 终端底部状态栏: `KodaX | CODE+think,auto | <session_id> | <provider>/<model>`

#### 测试 2: 会话恢复 (-c) 时 Banner 显示
...
```
**预期**:
1. 显示 `[Continuing session: <id>]`
2. **Banner 正常显示** (不会因为恢复会话而缺失)
3. 状态栏显示恢复的 session ID

#### 测试 3: MessageList 显示和流式状态
...
```
**预期**:
1. 用户消息显示 `You [时间戳]` + 消息内容
2. AI 响应期间显示 **动画 Spinner** (`⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏`)
3. Thinking 过程显示 `[Thinking] (X chars)` (X 为字符计数)
4. 工具执行时显示 `[Tool] <toolname> (X chars)`

#### 测试 4: Thinking 字符计数和 [Thinking] 标记

```bash
kodax
# 使用支持 thinking 的 provider
> 分析一下这个项目的架构
```
**预期**:
1. AI thinking 期间显示 Spinner 动画
2. 显示 `[Thinking]` 前缀标记
3. 显示实时字符计数，如 `(123 chars)`
4. thinking 结束后显示助手回复
...
```
**预期**:
1. 状态栏中间显示 `⏳ Read` (当前工具名)
2. MessageList 底部显示 `[Tool] Read (X chars)` (输入参数字符数)
3. 工具完成后状态恢复
...
```
**预期**:
- 状态栏显示 `CODE+think,auto`
- 仅 think 开启: `CODE+think`
- 仅 auto 开启: `CODE+auto`
- 都关闭: `CODE`
...
#### 测试 9: Backspace 和 Delete 键

```bash
kodax
# 测试 Backspace
> 输入 "hello"，光标在末尾，按 Backspace
**预期**: 删除 'o'，显示 "hell"

# 测试 Delete
> 输入 "hello"，光标移到 'h' 后，按 Delete
**预期**: 'e' 被删除，显示 "hllo"
```
...
ls ~/.kodax/sessions/*.jsonl | tail -1
```
**预期**: 新 .jsonl 文件被创建

### 12.2 流式状态详细测试

#### 测试 F1: Thinking 字符计数更新

```bash
kodax
> 请详细分析 TypeScript 的类型系统
```
**预期**:
- Spinner 动画持续旋转
- 字符计数实时递增: `(100 chars)` → `(250 chars)` → ...
- 计数每收到 thinking delta 就更新

#### 测试 F2: 工具输入字符计数

```bash
kodax
> 搜索所有 .ts 文件中的 interface 定义
```
**预期**:
- 显示 `[Tool] Grep (X chars)`
- X 随工具输入参数增长

### 12.3 已知限制

- **SuggestionsDisplay**: 已创建但未集成到 InputPrompt
- **外部编辑器**: Ctrl+O 功能未实现
- **粘贴保护**: 大文本粘贴保护未实现
- **Token 使用量**: 状态栏 Token 计数未实现

---

## 13. 更新后的功能对比

| 功能 | Claude Code | Gemini CLI | KodaX (当前) | KodaX (目标) |
|------|-------------|------------|--------------|--------------|
| 多行输入 | ✅ | ✅ | ✅ | ✅ |
| 消息列表渲染 | ✅ | ✅ | ✅ | ✅ |
| 流式响应 | ✅ | ✅ | ✅ | ✅ |
...
### 13.1 Phase 6.3 新增功能 (2026-02-20)

| 功能 | 描述 | 文件 |
|------|------|------|
| Thinking 字符计数 | 显示 `[Thinking] (X chars)` 实时更新 | `MessageList.tsx`, `StreamingContext.tsx` |
| 工具执行状态 | 显示 `[Tool] <name> (X chars)` | `MessageList.tsx`, `StreamingContext.tsx` |
| 状态栏模式指示 | `CODE+think,auto` 格式 | `StatusBar.tsx` |
| 状态栏当前工具 | 中间显示 `⏳ <toolname>` | `StatusBar.tsx` |
| Banner 稳定性 | `-c` 恢复会话时 Banner 正常显示 | `InkREPL.tsx` |

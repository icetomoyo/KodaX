...
**解决方案**:
1. 将 `showBanner` 从状态改为常量 `true` - 保持布局稳定
2. 添加 `patchConsole: false` - 允许命令输出正常工作
3. 删除未使用的 `tokenUsage` 状态和 `printStartupBanner` 函数

**预期效果**:
- Banner 始终显示，随内容向上自然滚动（类似 Claude Code）
- 命令输出正确追加
- 终端滚动历史完整保留

**文件改动**:
- `src/ui/InkREPL.tsx` - 移除状态切换，保持布局稳定
...
| 5.4.2 移除未使用代码 | P1 | `InkREPL.tsx` | ✅ 已完成 |

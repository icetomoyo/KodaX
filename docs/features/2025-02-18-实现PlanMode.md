...
>
> **实现摘要**: Core 层仅增加 `beforeToolExecute` 钩子，CLI 层实现计划生成、存储、展示和执行。支持计划持久化和中断恢复。
>
> **设计原则**：Core 层极简，CLI 层实现全部复杂逻辑
...
KodaX 目前对于复杂任务会直接执行，缺乏**执行前的计划确认环节**：

1. **不可预测性**：用户不知道 AI 将要执行哪些操作
2. **错误成本高**：AI 的错误操作可能直接修改文件
3. **无法中断恢复**：长时间任务中断后需要重新开始

### 用户场景

- **重构项目**：多文件修改，希望看到完整变更计划
- **安全审查**：关键环境需要人工审核每步操作
- **长时间任务**：需要中断后能恢复继续执行
...
1. **Core 层极简**：只增加一个钩子，其余逻辑在 CLI 层
2. **文本计划**：不依赖 JSON 结构化，避免解析失败
3. **中断恢复**：支持保存计划、恢复执行
4. **逐步确认**：每步执行前可确认/跳过
...
| 方案 | 描述 | 优点 | 缺点 | 决策 |
|------|------|------|------|------|
| **A. JSON 结构化** | AI 输出 JSON，程序解析 | 结构化、可编程 | 依赖 AI 输出可靠 JSON，脆弱 | ❌ 不采用 |
| **B. 文本计划** | AI 输出文本，简单解析 | 灵活、容错、易调试 | 需要简单解析逻辑 | ✅ **采用** |
| **C. Core 层实现** | Core 层包含计划逻辑 | 统一处理 | 违背 Core 极简原则 | ❌ 不采用 |
| **D. CLI 层实现** | CLI 层包含全部逻辑 | Core 保持极简，CLI 灵活 | 需要更多 CLI 代码 | ✅ **采用** |
...
export interface KodaXToolExecutionContext {
  ...
  beforeToolExecute?: (tool: string, input: Record<string, unknown>) => Promise<boolean>;  // 新增
}
...
| 任务 | 文件 | 改动 |
|------|------|------|
| 1 | `src/kodax_core.ts` | `KodaXToolExecutionContext` 新增 `beforeToolExecute` 钩子 |
| 2 | `src/kodax_core.ts` | `executeTool` 函数调用钩子 |
...
| 任务 | 文件 | 改动 |
|------|------|------|
| 3 | `src/cli/plan-storage.ts` | 新增：计划存储实现 |
| 4 | `src/cli/plan-mode.ts` | 新增：Plan Mode 主逻辑 |
| 5 | `src/interactive/commands.ts` | 新增 `/plan` 命令 |
...
| 决策 | 选择 | 理由 |
|------|------|------|
| JSON vs 文本计划 | **文本** | 避免 AI JSON 不可靠问题 |
| Core vs CLI 实现 | **CLI** | 保持 Core 极简，CLI 灵活 |
| 持久化 | **支持** | 中断恢复是核心价值 |
| 步骤级确认 | **支持** | 通过 `beforeToolExecute` 钩子 |

---

## 7. 与 Claude Code 的对比

| 功能 | Claude Code | KodaX (本方案) |
|------|-------------|----------------|
| Plan Mode 触发 | `/plan` | `/plan` 命令 |
| 计划展示 | 结构化列表 | 文本列表 |
| 步骤选择 | 支持 | 执行前确认 |
| 中断恢复 | 支持 | **支持（计划持久化）** |
| 计划持久化 | 不支持 | **支持** |
| Core 层复杂度 | 高 | **极简（1 个钩子）** |
...
Steps:
  ...
  3. ✏️  Create auth middleware - src/middleware/auth.ts
  4. ✏️  Create auth routes - src/routes/auth.ts
  5. 🔧 Add auth routes - src/app.ts
...
Steps:
  ...
  3. ✏️  Create auth middleware
  4. ✏️  Create auth routes
  5. 🔧 Add auth routes
...
| 风险 | 可能性 | 应对措施 |
|------|--------|----------|
| 计划解析不准确 | 中 | 优化提示词，保持解析逻辑简单 |
| 计划文件损坏 | 低 | try-catch，损坏时重新生成 |
| 与现有功能冲突 | 低 | 完善的测试覆盖 |

**回滚方案**：
- Core 层钩子为可选，不影响现有功能
- 删除 CLI 层 Plan Mode 相关文件即可完全禁用

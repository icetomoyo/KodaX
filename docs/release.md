...
- Custom OpenAI-compatible reasoning is negotiated from `auto`: rejected
  ...
  `reasoning` and structured `reasoning_details` replay only to the same
  endpoint and model.
- Timeout and cancellation errors keep native DOMException semantics, nested
  causes, and redacted error stacks across scoped Provider credentials; typed
  timeouts stay recognizable through Provider wrapping and recovery.
- KodaX's default `NODE_ENV` is stripped from user shell command and probe
  environments, so `npm install` and `npm ci` no longer implicitly omit
  devDependencies; KodaX's own production mode and explicit user values stay
  authoritative.

All root/workspace package versions and lockfile entries are `0.7.96-rc.8`.
The feature-design submodule, public guides, architecture documents, and
`kodax_manual` track this release. Historical release records retain their versions.
...
1. Config templates, strict source/test typechecks, manual documentation
   tests, reasoning negotiation regressions
   (`packages/coding/src/agent-runtime/reasoning-fallback.test.ts`,
   repl effort label suites), NODE_ENV shell environment regressions
   (`packages/coding/src/shell-execution/environment.test.ts`,
   `packages/coding/src/tools/bash.node-env.test.ts`,
   `src/kodax_bootstrap.test.ts`), A2A file-output regressions, image
   validation regressions, and package builds pass.
2. GitHub CI passes on the exact release commit across Node 20/22, Windows
   packaged Electron, Windows shell contracts, and Linux/macOS native platforms.
3. Push the reachable feature-design submodule commit before the parent commit.
4. Tag the green commit `v0.7.96-rc.8`; the Release workflow must produce five
   platform archives, the universal npm tarball, and SHA256SUMS, with every job green.
5. Publish the GitHub pre-release with release notes. Leave npm publication
   to the maintainer: set `npm_config_tag=rc` in the shell environment, then
   run `node scripts/release.mjs` to download and verify the exact universal
...
## v0.7.96-rc.7 release preparation

Release state: `v0.7.96-rc.7` was the GitHub pre-release for the exact
tagged commit — the seventh release candidate of the v0.7.96 line, returning
eligible A2A file outputs from successful writes. The tag-triggered Release
...
On top of rc.6 it includes:

- Eligible A2A file outputs are returned from successful writes in both
  coding and managed execution, independently of conversation compaction and
  the memory artifact ledger; existing publication limits and legacy
  executors are preserved, and HTML/HTM outputs are recognized as `text/html`.

All root/workspace package versions and lockfile entries are `0.7.96-rc.7`.
The feature-design submodule, public guides, architecture documents, and
`kodax_manual` track this release. Historical release records retain their versions.
...
1. Config templates, strict source/test typechecks, manual documentation
   tests, A2A file-output regressions (`src/a2a/file-outputs.test.ts`,
   `src/a2a/a2a.test.ts`, `packages/coding/src/agent.written-files.test.ts`,
   `src/runtime-daemon/server.test.ts`), Windows process-cleanup regressions,
   Issue 335 image-validation regressions, bundled archive probes, and
   package builds pass.
2. GitHub CI passes on the exact release commit across Node 20/22, Windows
   packaged Electron, Windows shell contracts, and Linux/macOS native platforms.
3. Push the reachable feature-design submodule commit before the parent commit.
4. Tag the green commit `v0.7.96-rc.7`; the Release workflow must produce five
   platform archives, the universal npm tarball, and SHA256SUMS, with every job green.
5. Publish the GitHub pre-release with release notes. Leave npm publication
   to the maintainer: set `npm_config_tag=rc` in the shell environment, then
   run `node scripts/release.mjs` to download and verify the exact universal
...
Release state: `v0.7.96-rc.6` was the GitHub pre-release for the exact
...
On top of rc.3 it includes:

- New Session event journals initialize at sequence zero under the existing
  lock instead of scanning unrelated Run logs, removing a synchronous startup
  stall that could push concurrent Session history reads past their
  15-second timeout.
- Cached sequence floors are bound to their journal epoch; a missing or
  corrupt sequence file recovers the durable log maximum even when another
  live Runtime holds an older cached floor, preventing duplicate event
  sequences and lost replay progress. Valid cursors keep the constant-cost path.
- Startup prewarm serves both Full RepoIntel routing and preturn from the
  same complete result.

All root/workspace package versions and lockfile entries are `0.7.96-rc.4`.
The feature-design submodule, public guides, architecture documents, and
`kodax_manual` track this release. Historical release records retain their versions.
...
1. Config templates, strict source/test typechecks, manual documentation
   tests, Issue 334 session journal/sequence recovery regressions
   (`docs/test-guides/ISSUE_334_v0.7.96_REGRESSION_GUIDE.md`,
   `packages/coding/src/repo-intelligence/runtime-prewarm.test.ts`),
   shell cleanup and shared-probe cancellation regressions, trusted-text
   permission regressions, FEATURE_299 stop-admission regressions, and
   package builds pass.
2. GitHub CI passes on the exact release commit across Node 20/22, Windows
   packaged Electron, Windows shell contracts, and Linux/macOS native platforms.
3. Push the reachable feature-design submodule commit before the parent commit.
4. Tag the green commit `v0.7.96-rc.4`; the Release workflow must produce five
   platform archives, the universal npm tarball, and SHA256SUMS, with every job green.
5. Publish the GitHub pre-release with release notes. Leave npm publication
   to the maintainer: set `npm_config_tag=rc` in the shell environment, then
   run `node scripts/release.mjs` to download and verify the exact universal
...
Release state: `v0.7.96-rc.3` was the GitHub pre-release for the exact
...
On top of rc.1 it includes:

- A first `sessions.cancel` request whose `expectedRunId` binding is already
  terminal is rejected atomically inside the Runtime before a new Stop
  frontier is published — typed `code: 'conflict'`, `denialSource:
  'stale_run'`, `retryable: false` — so delayed requests cannot stop
  successor Runs. Accepted request replay, partial-delivery recovery, and
  restart replay are preserved.

All root/workspace package versions and lockfile entries are `0.7.96-rc.2`.
The feature-design submodule, public guides, architecture documents, and
`kodax_manual` track this release. Historical release records retain their versions.
...
1. Config templates, strict source/test typechecks, manual documentation
   ...
   stop-admission/extension/text-transaction regressions
   (including `src/sdk-runtime.stop-admission.test.ts`), and package builds pass.
2. GitHub CI passes on the exact release commit across Node 20/22, Windows
   packaged Electron, Windows shell contracts, and Linux/macOS native platforms.
3. Push the reachable feature-design submodule commit before the parent commit.
4. Tag the green commit `v0.7.96-rc.2`; the Release workflow must produce five
   platform archives, the universal npm tarball, and SHA256SUMS, with every job green.
5. Publish the GitHub pre-release with release notes. Leave npm publication
   to the maintainer: set `npm_config_tag=rc` in the shell environment, then
   run `node scripts/release.mjs` to download and verify the exact universal
...
Release state: `v0.7.96-rc.1` was the GitHub pre-release for the exact
...
On top of beta.6 it includes the multimodal and interrupt-identity fixes:
...
- text/image tool results survive direct and managed dispatch, constructed
  ...
  adapters; MCP images persist as attachments, and image-capable OpenAI
  providers receive tool images after valid paired responses with matching
  prompt-cache diagnostics;
- local execution failures and Worker error codes keep their typed
  classification through child results, Actor metadata, and Runtime/daemon
  status;
...
Release state: `v0.7.96-beta.6` is the GitHub pre-release at `1d4a7a7d`,
with the diagnostics wire-fidelity work at `31eaa7e4`. Beta.6 is a
review-hardening pass over the beta.5 diagnostics. Prompt-cache
diagnostics now mirror the provider wire: the Anthropic diagnostic repair
mirrors the provider's pairing semantics (a retained orphan `tool_use` is
answered with the interrupted-tool marker inside its pairing scope, real
non-adjacent answers are hoisted, foreign and duplicate results are
dropped), the OpenAI diagnostic repair gains the same full pairing scope,
and assistant projection keeps source order to match the order-preserving
serialization. A 224-case wire contract suite proves the projected
diagnostic envelope equals the real provider request across every built-in
and custom Anthropic/OpenAI provider, including retained orphan calls that
must stay distinct from a thinking-only placeholder. The native SDDL
auto-inherited flag normalization parses the flag field instead of a blind
string replace, with its own regression test. `@kodax-ai/llm` exports
`KODAX_INTERRUPTED_TOOL_RESULT_MARKER` as the shared wire-contract
constant. All beta.5 contracts are retained. It predates the consolidated
`v0.7.96-beta.7` multimodal and interrupt-identity fixes.

## v0.7.96-beta.5 release preparation

Release state: `v0.7.96-beta.5` is the GitHub pre-release at `15a01cde`,
with the DeepSeek migration at `b25c5142` and the setup-generation-11 work
at `a5c23da1`. Beta.5 carries two change streams. First, the built-in `deepseek` provider
...
policy and all earlier contracts are retained. It predates beta.6's
diagnostics wire-fidelity hardening.

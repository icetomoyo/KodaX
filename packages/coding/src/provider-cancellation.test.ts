...
import {
  ...
  createProviderCredentialLeaseScope, runWithProviderCredentialLeaseScope,
...
describe.each(factories)('%s cancellation through Runner', (_name, factory) => {
  ...
  describe.each(['ambient', 'leased'])('%s credentials', (credentialMode) => {
    it.each(methods)('%s reports user cancellation without provider recovery', async (method) => {
      ...
      const client = anthropic ? { messages: { create: request } }
      ...
      if (anthropic || openai) Reflect.set(provider, 'buildClient', () => client);
      const scope = createProviderCredentialLeaseScope({
        allowedProviders: [provider.name], acquire: async () => 'fake-cancellation-secret',
      });
      const operation = () => Runner.run(createAgent({ name: 'cancel-test', instructions: 'test' }), 'hello', {
        tracer: null, abortSignal: controller.signal,
        llm: async () => {
          await withProviderRequestCredential(provider.name, 'primary', controller.signal, (signal) =>
            provider[method]([{ role: 'user', content: 'hello' }], [], 'system', undefined, undefined, signal));
          return 'should not complete after cancellation';
        },
      });
      const run = (credentialMode === 'leased'
        ? runWithProviderCredentialLeaseScope(scope, operation) : operation()).finally(() => scope.close());

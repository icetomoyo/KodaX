...
async function executeRunManagedTask(
  ...
  // FEATURE_084 Shard 6d (v0.7.26): AMA now runs through the Runner-driven
  // path by default. SA mode stays on the direct fast-path. The legacy
  // orchestration state machine is removed in this release.
  const agentMode = resolveManagedAgentMode(options);
  if (agentMode === 'sa') {
    const intentGate = inferIntentGate(prompt);

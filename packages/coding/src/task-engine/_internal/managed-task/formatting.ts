/**
 * Pure formatting helpers restored from v0.7.22 task-engine (FEATURE_079
 * Slice 5 + Slice 8). Each function is a pure string-building function of
 * its public `KodaX*` type inputs — no side effects, no state, no dependency
 * on task-engine-local types.
 *
 * Re-added v0.7.26 during parity audit: these formatters are reused by
 * `createRolePrompt` to inject contract / metadata / tool-policy /
 * verification / skill / scout-handoff context into every role prompt.
 * Without them the Runner-driven path only sees the role-name header and
 * misses the full task context legacy prompts carried.
 */

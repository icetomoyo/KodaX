 ...
 *
 * `createRolePrompt` extracted from task-engine.ts. Zero-behavior-change move.
 *
 * This is the largest pure extraction in FEATURE_079: ~480 lines of pure string
 * assembly that builds the system prompt for each managed role
 * (scout/planner/generator/evaluator/direct). It depends on:
 *
 * - Role-prompt context types (`ManagedRolePromptContext`, `ScoutScopeHint`)
 *   and scope inference (`inferScoutMutationIntent`, `isReviewEvidenceTask`)
 *   from `./role-prompt-types.js` — these were previously co-located private
 *   types/helpers in task-engine.ts.
 * - Formatters from `../formatting.js` — all pure string-building.
 * - Repo-intelligence working-tool predicate from `../../../tools/index.js`.
 * - Managed-protocol block-name / tool-name constants from
 *   `../../../managed-protocol.js`.
 *
 * The function signature matches the pre-extraction version exactly, including
 * the unused `workerId` parameter (preserved to keep all call sites in
 * task-engine.ts unchanged). See `// eslint-disable-next-line` note below.
 */
 ...
 *
 * `workerId` is kept on the signature (matching the pre-extraction version)
 * even though the body does not read it — some upstream callers thread it
 * through as a hook for future role-specific wording, and removing it would
 * churn every call site for no present benefit.
 */

...
const BUILTIN_TOOL_DEFINITIONS: LocalToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      ...
      + 'ANCHOR WARNING: because edits compose, when edits[k] deletes or rewrites a region, '
      + 'any text inside that region can no longer be used as an old_string anchor by '
      + 'edits[k+1..]. If later edits need to reference text that an earlier edit overlaps, '
      + 'either shrink the earlier edit so it preserves that anchor, or merge them into one edit. '

...
const BUILTIN_TOOL_DEFINITIONS: LocalToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      ...
      + 'being a valid anchor for edits[k+1..]. Shrink the earlier edit or merge them. '
      + 'UNIQUENESS RULE: each `old_string` must be unique in the WHOLE current file, not just in '
      + 'the window you last read. Short snippets from a narrow `read` are the top cause of '
      + '"matched N places" errors — widen the anchor with a nearby unique landmark (heading, '
      + 'function signature, distinctive comment), or set `replace_all: true` if every occurrence '
      + 'should change. '

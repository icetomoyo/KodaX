...
const { askInputMock, runManagedTaskMock } = vi.hoisted(() => ({
  askInputMock: vi.fn(),
...
// The product-client refactor routes unbound classic turns through the local
// managed-task engine instead of a runtimeRunner wrapper, so the display
// events are captured at that execution seam.
vi.mock('@kodax-ai/coding', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@kodax-ai/coding')>();
  return {
    ...actual,
    runManagedTask: runManagedTaskMock,
  };
});
...
async function driveOneTurn(events?: KodaXEvents): Promise<KodaXEvents[]> {
  ...
  runManagedTaskMock.mockReset();
  runManagedTaskMock.mockImplementation(async (options: { events?: KodaXEvents }) => {
    return fakeTurnResult();
  });
  const capture = (options: { events?: KodaXEvents }) => {
    captured.push(options.events ?? {});
    return fakeTurnResult();
  };
  const captured: KodaXEvents[] = [];
  runManagedTaskMock.mockImplementation(async (options: { events?: KodaXEvents }) => capture(options));

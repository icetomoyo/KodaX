...
import {
  DISCOVERY_QUESTIONS,
  appendAlignmentEntry,
  buildFallbackFeatureListFromAlignment,
  chooseAlignmentField,
  chooseOrInput,
  detectAlignmentField,
  formatStage,
  getRecommendedNextStep,
  getWorkflowState,
  isExecutionStage,
  isRemovalInstruction,
  looksLikeExplicitAlignmentFieldEdit,
  normalizeAlignment,
  removeAlignmentEntry,
  stripAlignmentEditPrefix,
...
async function projectBrainstorm(
  ...
  let alignment: ProjectAlignment = await storage.readAlignment() ?? createProjectAlignment(scopePrompt);
  if (alignment.sourcePrompt.trim() === '') {

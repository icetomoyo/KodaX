...
import type { SessionPickerItem } from '../ui/SessionPicker.js';
import { countResumableSessionItems } from './resumable-session.js';
...
async function collectActiveSessionFiles(
  ...
  const legacyFiles = await readProjectSessionFiles(sessionsDir);
  return [
...
async function scanResumeSessions(
  ...
  const { canonicalProjectRoot, compatibleRoots } = await resolveSessionDiscoveryRoots(options.projectRoot);
  const projectDir = path.join(sessionsDir, deriveProjectKeyFromRoot(canonicalProjectRoot).key);
  ...
  if (maintainIndex && projectManifestMissing && primaryProjectFilesMatch) {
  ...
  if (maintainIndex && indexedEntries === undefined && indexProjectDirTrusted) {
    try {
  ...
  return candidates.slice(0, options.limit ?? Number.MAX_SAFE_INTEGER).map(({
...
/** Bare resume is a read-only, bounded metadata snapshot. */
export function listCliResumeSessions(options: ListCliResumeSessionsOptions): Promise<SessionPickerItem[]> {
  return scanResumeSessions(options, false);
}

/** Normal storage owns certification and rebuilding; the picker never writes. */
export async function maintainProjectResumeIndex(options: ListCliResumeSessionsOptions): Promise<void> {
  await scanResumeSessions(options, true);
}
export async function resolveSessionDiscoveryRoots(projectRoot: string) {
  const projectIdentity = await resolveWorkspaceProjectIdentity({ cwd: projectRoot });
  ...
  return { canonicalProjectRoot, compatibleRoots };
}

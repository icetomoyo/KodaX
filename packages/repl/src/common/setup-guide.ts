...
export function renderSetupGuide(input: RenderSetupGuideInput = {}): string {
  ...
  return [
    ...
    '  Interactive Windows CLI startup verifies the installed generation once. A missing or',
    '  stale generation runs the same setup boundary before the REPL; concurrent startups',
    '  converge through the setup-only lock, and ready state stays silent.',
    '  Headless, print-mode, daemon, and SDK startup never trigger UAC automatically.',
    '  Ordinary tool calls never run setup, wait on its lock, or repeat an installer.',
    '  In the REPL, use `/sandbox` to inspect readiness.',
    '  Run `kodax sandbox doctor` directly in a host terminal, not through the Bash tool.',
    '  Activate explicitly from a host terminal with `kodax sandbox setup`.',

...
describe('isolated Skill Script platform documentation', () => {
  it('marks the protocol-10 route POSIX-only and Windows unavailable', () => {
    for (const relativePath of [
      ...
      expect(source, relativePath).toMatch(/protocol 10[\s\S]{0,160}POSIX-only/u);
    ...
    expect(chineseReadme).toMatch(/native protocol 10[\s\S]{0,160}仅支持 POSIX/u);

...
import { connectKodaXRuntime, ensureKodaXRuntime, waitForRuntimeDaemonShutdown } from '../dist/sdk-runtime.js';
...
test('bundled daemon routes manual and managed compaction through the v2 broker', { timeout: 60_000 }, async (t) => {
  ...
  const connectionOptions = {
    homeDir, sessionsDir, daemonOrphanExitMs: 1000,
  ...
  runtime = await ensureKodaXRuntime(connectionOptions);
  try {
    ...
    for (const { name, protocol } of customProviders) {
      ...
      assert.equal(request.target.kind, 'operation');
      assert.equal(request.target.operation, 'session.compact');
      assert.match(request.target.operationId, /^compact_[a-f0-9]+$/);
    }

...
import { createKodaXRuntime, connectKodaXRuntime, getKodaXRuntimeOwnerState,
...
async function openRuntime(fixture, mode) {
  ...
  const opening = mode === 'worker'
    ? createKodaXRuntime({ ...common, isolation: 'worker' })
    : connectKodaXRuntime({ ...common, autoStart: true, daemonOrphanExitMs: 1000,
    ...
    async stop() {
      ...
      try {
        if (mode === 'daemon') {
          ...
          owner ??= state.owner;
          await runtime.daemon.stopForInline({ expectedRuntimeId: state.runtimeId,
            expectedRevision: state.revision, expectedOwnerPolicyRevision: state.ownerPolicy.revision });
        }
...
async function resumeReviewInWorker(fixture, session, jobId) {
  fixture.endpoint.state.holdReview = false;
  fixture.endpoint.state.holdPrimary = false;
  const replacement = await openRuntime(fixture, 'worker');
...
for (const mode of ['worker', 'daemon']) {
  ...
  test(`bundled ${mode} cancels ambient-credential production review HTTP and the next Runtime resumes its durable job`,
    { timeout: 60_000 }, async (t) => {
      ...
      try {
        ...
        // Worker uses its production default 2s shutdown deadline, with no override.
        ...
        await resumeReviewInWorker(fixture, session, job.jobId);
...
test('bundled scoped daemon aborts a Run with an active startup review before quit and recovery',
  { timeout: 100_000 }, async (t) => {
    ...
    try {
      ...
      await resumeReviewInWorker(fixture, first.session, job.jobId);
      assert.deepEqual(fixture.endpoint.state.errors, []);
    } catch (error) {
      fixture.evidence.keep = true;
      t.diagnostic(JSON.stringify({ reviewRequests: fixture.endpoint.state.reviews.length,
